    ...
    if request.method == 'POST':
        ...
        for index, file in enumerate(files):
            ...
            if file.name.lower().endswith('.pdf'):
                ...
                # Set the timeout based on the number of pages (e.g., 60 seconds per page)
                timeout_per_page = 60
                total_timeout = num_pages * timeout_per_page

                # Process PDF with async_batch_annotate_files
            ...
            # Save the extracted text to a DOC file directly
            media_root = settings.MEDIA_ROOT
            if not os.path.exists(media_root):
                os.makedirs(media_root)

            # Create a unique filename for each DOC file based on the original file name without the ".pdf" extension
            base_name = os.path.splitext(file.name)[0]
            doc_file_path = os.path.join(media_root, f'{base_name}.docx')  # Use original filename for DOCX

            # Write the extracted text to the DOC file in an optimized way
            document = Document()
